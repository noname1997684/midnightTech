...
const Form = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      toast.error("Error fetching brand data:", error.message);
    }
  };
  ...
  const handleCreate = async (e) => {
    setLoading(true);
    try {
      await createNewBrand(data, picURL);
  ...
  const handleUpdate = async (e) => {
    setLoading(true);
    try {
      await updateBrand(data, picURL);
      toast.success("Brand updated successfully");
      setData(null);
      setPicURL(null);
      router.push("/admin/brands");
    } catch (error) {
      toast.error("Error updating brands:", error.message);
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="flex flex-col gap-1">
          ...
          {picURL && (
            <div className="flex justify-center items-center p-3">
              <img className="h-16" src={picURL} alt="brand-image" />
            </div>
          )}
          <input
          required
