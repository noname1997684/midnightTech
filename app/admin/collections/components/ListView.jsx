    ...
    <div className='md:pr-5 md:px-0 px-5 rounded-xl flex-1 flex flex-col gap-5'>
        ...
        <table className='border-separate border-spacing-y-3'>
          ...
          {collections?.map((collection,index) => (
           <Row index={index} collection={collection} key={collection.id}/>
          ))}
          </tbody>
        </table>
    </div>
     ...
     <tr>
              ...
              <td className='border-y bg-white px-3 py-2'>
                <div className='flex justify-center'>
                  <img className='h-10  object-cover' src={collection.imageURL} alt={collection.title} />
                </div>
              </td>
              <td className='border-y bg-white px-3 py-2'>{collection.title}</td>
              <td className='border-y bg-white px-3 py-2'>{collection.products.length}</td>
